...
class ODA2LunaLayer(nn.Module):
    ...
    def __init__(self,
        ...
        # cross attention 1
        self.q_cross1 = nn.Linear(aux_dims, min(aux_dims, in_dims))
        self.k_cross1 = nn.Linear(in_dims, min(aux_dims, in_dims))
        self.v_cross1 = nn.Linear(in_dims, aux_dims)
        self.o_cross1 = nn.Linear(aux_dims, aux_dims)
        self.norm_cross1 = nn.LayerNorm(aux_dims)

        # cross attention 2
        self.q_cross2 = nn.Linear(in_dims, min(aux_dims, in_dims))
        self.k_cross2 = nn.Linear(aux_dims, min(aux_dims, in_dims))
        self.v_cross2 = nn.Linear(aux_dims, out_dims)
        self.o_cross2 = nn.Linear(out_dims, out_dims)
        self.out_cross2 = nn.Identity()
    ...
    def forward(self, x: torch.Tensor, aux: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        weight_c = self.out_cross2(weight_c)
...
class ODA2LunaClsDecoder(nn.Module):
    """Basic Luna decoder stack, classification."""
    ...
    def __init__(self,
        ...
        convbn_kwargs = dict(act_layer=act_layer, use_gn=True, num_groups=64)
        ...
        self.block4 = nn.Sequential(
            ...
            nn.Conv2d(self.output_channels[0], num_aux, kernel_size=(3, 3), padding=(1, 1), padding_mode="replicate")
        )

        self.bins = nn.Sequential(
            nn.Linear(aux_dims, aux_dims),
            nn.ReLU(),
            nn.Linear(aux_dims, aux_dims),
            nn.ReLU(),
            nn.Linear(aux_dims, 1),
        )
    ...
    def forward(self, features: List[torch.Tensor]) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        bin_probs = self.block4(c0)
        bin_probs = torch.softmax(bin_probs, dim=1)

        bin_widths = self.bins(aux)
        bin_widths = bin_widths.squeeze(2)  # (b, n_bins)
        bin_widths = F.elu(bin_widths, alpha=0.1) + 0.1
        # bin_widths = torch.relu(bin_widths) + 0.1
        bin_widths = bin_widths / torch.sum(bin_widths, dim=-1, keepdim=True)

        return bin_probs, bin_widths
