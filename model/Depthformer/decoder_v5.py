...
class DepthFormerDecoderV5(nn.Module):
    ...
    def __init__(self,
        ...
        self.key_query_dim = key_query_dim # 128
        ...
        self.vit_dims = [hidden_dim // 4, hidden_dim//2, hidden_dim]
        self.key_query_dims = [key_query_dim//16, key_query_dim//4, key_query_dim]
        self.out_channel_dims = [hidden_dim//16, hidden_dim//8, hidden_dim//4, hidden_dim//2, hidden_dim]
        self.vit_heads = [num_heads // 4, num_heads // 2, num_heads]

        self.vit_layers = nn.ModuleList([
            ViTLayer(self.vit_dims[i], self.key_query_dims[i], self.vit_heads[i], num_repeat=num_repeat, feedforward_dim=feedforward_dim,
        ...
        for i in range(num_layers):
            if i != num_layers - 1:
                in_ch = self.input_channels[i] + self.out_channel_dims[i + 1]
                out_ch = self.out_channel_dims[i]
            else:
                in_ch = self.input_channels[i]
                out_ch = self.out_channel_dims[-1]
        ...
        self.final_block = nn.Sequential(
            nn.Conv2d(self.out_channel_dims[0], 1, kernel_size=(1, 1)),
            nn.Sigmoid()
        )
