...
from .decoder_v4 import DepthFormerDecoderV4
...
class DepthformerV4(nn.Module):
    def __init__(self, backend, opt,
                 min_depth: float, max_depth: float):
        super(DepthformerV4, self).__init__()
        self.encoder = Encoder(backend)
        self.decoder = DepthFormerDecoderV4(
    ...
    def forward(self, x: torch.Tensor) -> Tuple[torch.Tensor, Tuple[torch.Tensor, ...]]:
        ...
        # depth_output = torch.clamp_max(depth_output, self.max_depth - self.min_depth) + self.min_depth
...
if __name__ == '__main__':
    ...
    net = DepthformerV4.build(opt={"hidden_dim": 256, "num_heads": 4, "img_size": (480, 640)},
