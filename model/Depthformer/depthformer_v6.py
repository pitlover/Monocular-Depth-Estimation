...
from .decoder_v6 import DepthFormerDecoderV6
...
class DepthformerV6(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> Tuple[torch.Tensor, Tuple[torch.Tensor, ...]]:
        ...
        bin_width = F.pad(bin_width, (1, 0), mode='constant', value=self.min_depth)
        bin_edges = torch.cumsum(bin_width, dim=1)

        centers = 0.5 * (bin_edges[:, :-1] + bin_edges[:, 1:])
        centers = centers.unsqueeze(-1).unsqueeze(-1)  # (b, n_bins, 1, 1,)

        depth_output = torch.sum(bin_cls * centers, dim=1, keepdim=True)  # (b, 1, h/2, w/2)

        # TODO return center too
        return depth_output, attn_weights
