...
from .decoder_v5 import DepthFormerDecoderV5
...
class DepthformerV5(nn.Module):
    def __init__(self, backend, opt,
                 min_depth: float, max_depth: float):
        super(DepthformerV5, self).__init__()
        self.encoder = Encoder(backend)
        self.decoder = DepthFormerDecoderV5(
            hidden_dim=opt["hidden_dim"],  # 1024?
            key_query_dim=opt["key_query_dim"],
            num_heads=opt["num_heads"],  # 4?
            input_channels=(24, 40, 64, 176, 2048),
    ...
    def forward(self, x: torch.Tensor) -> Tuple[torch.Tensor, Tuple[torch.Tensor, ...]]:
        ...
        decoder_input = (encoder_feat[4], encoder_feat[5], encoder_feat[6], encoder_feat[8], encoder_feat[12])
        depth_output, range_attn_maps = self.decoder(decoder_input)

        # depth_output pass through sigmoid [0, 1], so we rescale to real depth
        depth_output = (self.max_depth - self.min_depth) * depth_output + self.min_depth

        return depth_output, range_attn_maps
...
if __name__ == '__main__':
    ...
    net = DepthformerV5.build(opt={"hidden_dim": 512, "key_query_dim": 128, "num_heads": 4, "img_size": (480, 640)},
                              min_depth=0.001, max_depth=80.0)
    depth_pred, attn_weights = net(dummy_x)
