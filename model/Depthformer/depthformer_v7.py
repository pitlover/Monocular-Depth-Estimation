...
from .decoder_v7 import DepthFormerDecoderV7
...
class DepthformerV7(nn.Module):
    def __init__(self, backend, opt,
                 min_depth: float, max_depth: float):
        super(DepthformerV7, self).__init__()
        self.encoder = Encoder(backend)
        self.decoder = DepthFormerDecoderV7(
            hidden_dim=opt["hidden_dim"],  # 512?
            num_heads=opt["num_heads"],  # 8?
